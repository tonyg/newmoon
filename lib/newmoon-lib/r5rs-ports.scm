(define (display x . p)
  (if (null? p)
      (let-primitive ((d (specific-static "Newmoon.Primitive" "Display" object))) (d x))
      (let-primitive ((d (specific-static "Newmoon.Primitive" "Display" object output-port)))
	(d x (car p)))))
...
(define (newline . p)
  (if (null? p)
      (let-primitive ((n (specific-static "System.Console" "WriteLine"))) (n))
      (let-primitive ((n (specific-method output-port "WriteLine"))) (n (car p)))))
